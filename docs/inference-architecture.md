...
У каждой модели свой DAG, очередь и поды общие. Текущий TaskStore реализован в S3;
PG backend в коде пока отсутствует.

## 1. Общая архитектура

При старте под вызывает заглушку скачивания артефактов; веса загружает worker только после захвата задачи. Затем API принимает заявки, свободный worker забирает расчёты, а отдельный монитор следит за таймаутами. Очередь физически хранится в S3 — на схеме она включена в границу сервиса как его общий компонент.
...
sequenceDiagram
    ...
    par Обработка данных
        loop Для каждого чанка
            ...
            W->>Q: Периодически сохранить прогресс и metrics_util_*
        end
    and Подтверждение работы исполнителя
        loop Пока worker работает
            W->>Q: Heartbeat и актуальная статистика утилизации
        end
    and Сбор статистики на поде
        loop Каждую секунду до завершения close
    ...
    and Ожидание в Airflow
        loop До финального статуса
            ...
            Q-->>API: Статус, прогресс, статистика, причина ошибки
    ...
    W->>Q: Подтвердить DONE и сохранить статистику
    W->>W: close в finally, остановить сбор утилизации
    W->>Q: Сохранить окончательные metrics_util_* до следующей задачи
