...
def run_task(
    ...
    try:
        ...
        store.start_utilization(task_id, spec.device)
        ...
        if bundle is not None:
            try:
                ...
                bundle = None
        if utilization_started:
            try:
                store.finish_utilization(task_id)
            except Exception:
                logger.error(
                    f"Задача {task_id}: не удалось сохранить итоговую утилизацию\n"
                    + traceback.format_exc()
                )
        heartbeat_stop.set()
