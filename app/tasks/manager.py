...
(чтобы избежать гонки между двумя параллельными запросами /infer, прилетевшими почти одновременно).
"""
...
class TaskManager:
    ...
    def try_start_task(
        ...
        с её состоянием (для формирования 409-ответа с статусом и ETA).
        Иначе создаёт новую TaskState со статусом RUNNING и регистрирует
        cancellation-флаг, возвращает её вызывающему роуту, который
