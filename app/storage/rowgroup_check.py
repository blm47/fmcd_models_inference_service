...
def warn_on_oversized_row_groups(dataset, chunk_size: int) -> None:
    """
    Диагностический проход ПЕРЕД чтением через dataset.to_batches().

    Проходит по каждому part-файлу (fragment) датасета и проверяет его
    row group'ы через footer-метаданные (без чтения самих данных - это
    дёшево, читаются только footer каждого файла).
    ...
    один батч, независимо от значения chunk_size. Явно логируем warning
    с именем файла и точным размером, чтобы при повторном OOM не искать
    причину заново, а сразу понимать, какой файл виноват.
    """
    for fragment in dataset.get_fragments():
        ...
        if num_row_groups == 1:
            ...
            # они у файла неравномерные (например, последний row group
            # аномально большой)
